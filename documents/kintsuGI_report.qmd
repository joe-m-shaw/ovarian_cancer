...
```{r}
#| label: tbl-gi-tvar-glvar-all-results
#| tbl-cap: "All testing results, including fails and inconclusive results"
#| echo: FALSE

glvar_all_results <- gi_glvar_joined |> 
  group_by(data_source) |> 
  count() |> 
  rename("Patients with GI and germline NGS results" = n)

tvar_all_results <- gi_tvar_joined |> 
  group_by(data_source) |> 
  count() |> 
  rename("Patients with GI and tumour NGS results" = n)

tbl_gi_glvar_tvar_all <- glvar_all_results |> 
  left_join(tvar_all_results, by = "data_source") |> 
  rename("NGS result data source" = data_source) |> 
  adorn_totals()

knitr::kable(tbl_gi_glvar_tvar_all)
...
```{r}
#| label: tbl-gi-tvar
#| tbl-cap: "GI results and tumour testing results"
#| echo: FALSE

tbl_gi_tvar <- gi_tvar_joined |> 
