---
title: "Annotate Genomic Instability CSV files"
format: 
  docx:
...
source(here("scripts/connect_to_sql_server.R"))
source(here("functions/functions.R"))

```

```{r}
#| label: load-collated-csv-data
#| include: FALSE
...
# Add sample identifiers to CSV data

```{r}
#| label: find-sample-identifiers
#| include: FALSE
...
Some GI data relates to validations which were performed, some relates to the live clinical service. 
The script annotates each result as "validation" or "service" based on whether the worksheet is listed as a validation worksheet or not.
On one worksheet (WS138061) there were 2 validation samples (23047082 and 23053359) and the rest were live service samples.
...
There are `r nrow(same_name_df)` cases in which a patient has an identical name to another patient, which stresses the importance of using NHS number as an identifier.

```{r}
#| label: tbl-same-names
#| echo: FALSE

knitr::kable(same_name_df)
